...
<!-- Extended Schema Sections (10~14) -->
...
|------|-------|-------------|
| LandingPage | / | 메인 마케팅 페이지 |
| PricingPage | /pricing | 요금제 비교 |
| DocsPage | /docs | 개발자 문서 |
| LoginPage | /login | 로그인 |
| DashboardPage | /dashboard | 개발자 대시보드 |

### Naming Examples
- LandingPage > HeroSection > HeadlineBlock > TitleElement
- LandingPage > HeroSection > CTABlock > PrimaryButton
- LandingPage > FeaturesSection > FeatureCardBlock > IconElement
- PricingPage > TierSection > PlanCardBlock > PriceElement
- DashboardPage > MetricsSection > RevenueBlock > ChartElement
...
- **Layout**: full-width sections, centered content (max 1080px)

| Section | Block | Description |
|---------|-------|-------------|
| HeroSection | HeadlineBlock | 56px 타이틀 + 18px 서브타이틀 |
| HeroSection | CTABlock | Primary(purple) + Ghost 버튼 |
| FeaturesSection | FeatureCardBlock × 3 | 아이콘 + 제목 + 설명 카드 그리드 |
| BrandSection | DarkBrandBlock | #1c1e54 배경, 화이트 텍스트 |
| CodeSection | CodePreviewBlock | SourceCodePro 코드 프리뷰 + 블루 그림자 |
| SocialProofSection | LogoGridBlock | 고객사 로고 그리드 |
| CTASection | FinalCTABlock | 하단 CTA 반복 |
...
| Section | Block | Description |
|---------|-------|-------------|
| NavSection | SidebarNavBlock | 수직 네비게이션 |
| MetricsSection | RevenueBlock | 매출 차트 (tnum 숫자) |
| MetricsSection | VolumeBlock | 트랜잭션 볼륨 카드 |
| ActivitySection | EventListBlock | 최근 이벤트 테이블 |
...
- **Type**: Composite
- **Reusable**: yes
- **Variants**: default | with-code-preview | with-illustration

| Element | Type | Required | Props |
|---------|------|:---:|-------|
| Headline | Text | ✅ | size=48-56px, weight=300, font=sohne-var |
| Subtitle | Text | ✅ | size=18px, weight=300, color=#64748d |
| PrimaryButton | Button | ✅ | variant=primary, label (i18n) |
| GhostButton | Button | — | variant=ghost, label (i18n) |

### FeatureCard
- **Type**: Composite
- **Reusable**: yes
- **Variants**: icon-top | image-top | code-preview

| Element | Type | Required | Props |
|---------|------|:---:|-------|
| Icon | Image | ✅ | src, alt |
| Title | Text | ✅ | size=22px, weight=300 |
| Description | Text | ✅ | size=16px, weight=300, color=#64748d |
| LearnMoreLink | Link | — | label (i18n), href |
...
| Element | Type | Required | Props |
|---------|------|:---:|-------|
| StatusDot | Icon | — | color=#15be53 |
| Label | Text | ✅ | size=10px, weight=300, color=#108c3d, font-feature=tnum |
...
| Purple Hover | #4434d4 | --color-primary-hover | hover:bg-primary-hover |
| Deep Navy | #061b31 | --color-heading | text-heading |
| Body Slate | #64748d | --color-body | text-body |
| Label Slate | #273951 | --color-label | text-label |
| White | #ffffff | --color-surface | bg-surface |
| Brand Dark | #1c1e54 | --color-surface-dark | bg-surface-dark |
| Border Default | #e5edf5 | --color-border | border-default |
| Success Green | #15be53 | --color-success | text-success / bg-success |
| Ruby | #ea2261 | --color-accent-ruby | text-accent-ruby |
| Magenta | #f96bee | --color-accent-magenta | text-accent-magenta |

### Typography Tokens
| Role | CSS Variable | Tailwind Class |
|------|-------------|----------------|
| Display Hero | --text-display-hero | text-display-hero |
| Display Large | --text-display-lg | text-display-lg |
| Section Heading | --text-heading | text-heading-section |
| Body | --text-body | text-body |
| Caption | --text-caption | text-caption |
| Code | --text-code | text-code font-mono |

### Spacing Tokens
| Name | Value | CSS Variable | Tailwind Class |
|------|-------|-------------|----------------|
| space-1 | 4px | --space-1 | p-1 / gap-1 |
| space-2 | 8px | --space-2 | p-2 / gap-2 |
| space-3 | 12px | --space-3 | p-3 / gap-3 |
| space-4 | 16px | --space-4 | p-4 / gap-4 |
| space-5 | 20px | --space-5 | p-5 / gap-5 |

### Radius Tokens
| Name | Value | CSS Variable | Tailwind Class |
|------|-------|-------------|----------------|
| radius-sm | 4px | --radius-sm | rounded-sm |
| radius-md | 5px | --radius-md | rounded |
| radius-lg | 6px | --radius-lg | rounded-md |
| radius-xl | 8px | --radius-xl | rounded-lg |
...
### Key Map
| Key | Default (en) | ko | Context |
|-----|-------------|-----|---------|
| landing.hero.title | Payments infrastructure for the internet | 인터넷을 위한 결제 인프라 | HeroSection > Headline |
| landing.hero.subtitle | Millions of companies use Stripe... | 수백만 기업이 Stripe를 사용합니다... | HeroSection > Subtitle |
| landing.hero.cta_primary | Start now | 지금 시작하기 | CTABlock > PrimaryButton |
| landing.hero.cta_secondary | Contact sales | 영업팀 문의 | CTABlock > GhostButton |
| common.nav.signin | Sign in | 로그인 | Navigation > SignInLink |
| dashboard.metrics.revenue | Revenue | 매출 | MetricsSection > RevenueBlock > Title |
...
```json
{
  "landing": {
    "hero": {
      ...
      "subtitle": "Millions of companies use Stripe...",
