...
> spec-5-02 핵심 산출물 1/2. Designer (Dennis) 가 직접 Paper 에서 SettingsPage 를 그린 후 AI 가 추출하는 사이클을 통해 *원본 의도가 어떻게 보존/손실되는지* 측정한다.

## §1. 원본 의도 (Designer 직접 그리기 *전*에 선언)

> Designer 가 Paper 에서 SettingsPage 를 그리기 *전에* 본인의 의도를 명시적으로 선언한다. 이 선언이 추출 비교의 기준이 된다.

### 1.1 페이지 의도 한 줄

> "TaskFlow 의 Settings 는 *조용한 환경 설정* 톤. 자주 쓰는 토글이 위에, 위험한 액션 (계정 삭제) 은 가장 아래에. 그룹 사이는 명확히 분리되되 카드처럼 박스가 두드러지지 않게 — 정보가 surface 위에 직접 놓여 있는 느낌."
...
## §2. 추출 결과 비교 (Designer 직접 그리기 *후* 채움)

> Designer 가 Paper 에서 SettingsPage 를 그린 *후* AI 가 `design-extract/settings-overview.md` 로 추출. 그 결과를 §1 의 의도와 항목별 비교한다.
