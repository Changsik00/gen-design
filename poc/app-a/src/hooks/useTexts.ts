...
export function getLoginTexts(): LoginPageTexts {
  return {
    ...
    signupLink: t.login.signupPrompt.split("?")[1]?.trim() ?? "Sign up",
    socialGoogle: t.login.social.google,
    socialGithub: t.login.social.github,
    // 미사용 — DESIGN.md 앱 A 는 google/github 만. 호환 위해 빈 문자열.
    socialApple: "",
    socialKakao: "",
  };
}
...
export function getSignupTexts(): SignupPageTexts {
  return {
    ...
    loginLink: t.signup.loginPrompt.split("?")[1]?.trim() ?? "Sign in",
...
export function getDashboardTexts(): DashboardPageTexts {
  return {
    ...
    activityColumns: {
      // DESIGN.md §11 dash-overview 컬럼 의미는 task/assignee/status/updated.
      // studio 의 ActivityRowData 는 user/action/status/time 명명 (Phase 2).
      // 의미 모델 차이는 visual-comparison.md 에 drift 로 기록 — 여기서는
      // 화면에 보일 라벨만 DESIGN.md 와 일치시킴.
