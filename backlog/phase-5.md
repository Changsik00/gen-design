...
- **spec-4-02/03 이월 흡수** (2026-04-22, spec-4-03 rigor 반영): 앱 A 생성 과정에서 다음을 자연스럽게 검증한다.
  - **원본 의도 보존 검증 (최우선)** — Phase 4 가 증명 못한 부분. Designer 가 직접 Paper 에서 그린 LoginPage 를 AI 가 추출한 DESIGN.md 와 대조 — 의도 → 저장 → 복원 완전 사이클.
  - LoginPage **variant 확장** — modal / bottom-sheet variant 의 왕복 drift 측정 (spec-4-02 는 page variant 만 부분 검증).
  - **DashboardPage 왕복 drift 측정** — 데이터 집약 페이지에서의 왕복 정확도 (Phase 4 의 표기 정규화 패턴이 복잡 구조에서도 유지되는가).
  - 상호작용 state (hover / focus / disabled) 동기화 가능성.
- **연관 모듈**: `poc/app-a/`
