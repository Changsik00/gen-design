...
describe("chat-react determinism benchmark — 28 fixtures", () => {
  for (const f of fixtures) {
    it(`${f.name} — two runs produce identical output`, () => {
      ...
      if (!r1.ok || !r2.ok) {
        results.push({ fixture: f.name, pass: false, reason: "compile error" });
      ...
      results.push({ fixture: f.name, pass, reason: pass ? undefined : "hash mismatch" });
