...
describe("LoginScene variant switching", () => {
  it("page variant renders split-screen layout with branding panel", () => {
    const { container } = render(<LoginScene variant="page" texts={texts} />);
  ...
  it("both variants render the same form content (signup link)", () => {
    const { unmount } = render(<LoginScene variant="page" texts={texts} />);
  ...
  it("bottom-sheet variant renders form at bottom with drag handle", () => {
    const { container } = render(<LoginScene variant="bottom-sheet" texts={texts} />);
  ...
  it("bottom-sheet variant has no split-screen branding", () => {
    render(<LoginScene variant="bottom-sheet" texts={texts} />);
    expect(screen.queryByText("Admin Console")).not.toBeInTheDocument();
  });
});
