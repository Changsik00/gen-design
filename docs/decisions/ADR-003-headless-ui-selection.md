# ADR-003: Headless UI 라이브러리 선택 — shadcn/ui (Base UI) 확정
...
phase-1에서 shadcn/ui로 프로젝트를 셋업했으나, Headless UI 라이브러리 최종 선택은 유보 상태였다 (ADR-001 참조). 원래 phase-2에서 비교 구현할 계획이었다.

> **참고**: shadcn/ui v4+는 내부적으로 Radix UI가 아닌 **Base UI (`@base-ui/react`)**를 사용한다. Base UI는 Radix 팀이 만든 차세대 headless 라이브러리이다. `button.tsx`의 `import { Button as ButtonPrimitive } from "@base-ui/react/button"` 등에서 확인 가능.

후보:
- **shadcn/ui (Base UI 기반)** — 현재 셋업
- **React Aria (Adobe)** — 대안 후보

## 결정

**shadcn/ui (Base UI)를 확정한다.** 비교 구현 없이 결정한다.
...
Base UI(구 Radix)가 제공하는 접근성 수준은 이 프로젝트에 충분하다. React Aria의 더 세밀한 ARIA 패턴은 이 프로젝트에서 과잉이다.
...
- 향후 Base UI가 제공하지 않는 컴포넌트가 필요하면 직접 구현으로 보완
