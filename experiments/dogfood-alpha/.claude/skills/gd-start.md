...
```
안녕하세요. dogfood-alpha 디자인 시작을 도와드리겠습니다.
...
```
1. dogfood-alpha 은 어떤 서비스인가요? (한 문장)
2. 누가 쓸 건가요? (타깃 사용자)
```
...
```
좋습니다. 이제 3가지 중 어느 것부터 시작할까요?

A. /gd-token   — 색 / 토큰 (브랜드 색이 정해져 있다면 먼저)
B. /gd-design  — 디자인 컨벤션 (전체 톤 / 컴포넌트 어휘 정의)
C. /gd-chat    — 바로 첫 화면 만들기 (welcome / 로그인 등)

추천 순서: A → B → C (token 이 정해지면 모든 결정이 일관)
하지만 빨리 결과를 보고 싶다면 바로 C 로 가도 됩니다.
```

→ 사용자 답변 받으면 해당 스킬 호출 안내 (`/gd-token` 입력하라고).
